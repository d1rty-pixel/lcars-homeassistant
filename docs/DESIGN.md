...
- **Waste page** (inspired by LCARS "exterior overview" / "database" screens):
  ...
  - The space beside each value carries a graphic, never plain black (LCARS
    ...
    flash white briefly (2.5 % of a random 8-24 s cycle, user request); inactive ones stay still.
...
- **OPS page** follows the waste page: header outside/condition + number
  ...
  Next, Now in its pillar). Page title "Operations" (menu label stays "OPS"). Like the waste page, the top frames are open at the
